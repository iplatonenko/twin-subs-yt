...
// @version      2025.08.24.1240
// @icon         https://raw.githubusercontent.com/iplatonenko/twin-subs-yt/main/assets/logo.svg
// @downloadURL  https://raw.githubusercontent.com/iplatonenko/twin-subs-yt/dist/twin-subs-yt.user.js
// @updateURL    https://raw.githubusercontent.com/iplatonenko/twin-subs-yt/dist/twin-subs-yt.user.js
...
(o=>{if(typeof GM_addStyle=="function"){GM_addStyle(o);return}const r=document.createElement("style");r.textContent=o,document.head.append(r)})(' .custom-captions-overlay{z-index:999999;padding:8px 10px;background:#0000008c;border-radius:8px;color:#fff;line-height:1.6;font-size:24px;font-weight:500;-webkit-backdrop-filter:blur(2px);backdrop-filter:blur(2px);cursor:grab}.word-chip-wrapper{position:relative;display:inline-block}.word-chip{display:inline-block;padding:2px 6px;border-radius:6px;background:#ffffff1f;-webkit-user-select:text;user-select:text;cursor:default;outline:none}.word-popover{position:absolute;left:50%;bottom:calc(100% + 8px);transform:translate(-50%) scale(.98);transform-origin:bottom center;opacity:0;visibility:hidden;pointer-events:auto;background:#0f0f12e6;color:#fff;border:1px solid rgba(255,255,255,.15);border-radius:8px;padding:8px 10px;display:inline-block;width:max-content;min-width:160px;max-width:280px;line-height:1.4;font-size:16px;z-index:1000000;filter:drop-shadow(0 6px 20px rgba(0,0,0,.35));transition:opacity .12s ease,transform .12s ease,visibility .12s linear}.word-popover.is-open{opacity:1;visibility:visible;transform:translate(-50%) scale(1)}.word-popover:after{content:"";position:absolute;left:50%;top:100%;transform:translate(-50%);border:6px solid transparent;border-top-color:#0f0f12fa}.word-popover__rows{display:flex;flex-direction:column}.word-popover__row{display:flex;gap:8px;align-items:baseline}.word-popover__row+.word-popover__row{margin-top:6px;border-top:1px dashed rgba(255,255,255,.15);padding-top:6px}.word-popover__label{flex:0 0 auto;font-weight:700;letter-spacing:.04em;opacity:.85;min-width:28px}.word-popover__value{flex:1 1 auto;word-break:break-word;white-space:normal}.word-popover__actions{display:flex;justify-content:flex-start;gap:8px;margin-bottom:12px}.word-popover__btn{appearance:none;background:#ffffff14;color:#fff;border:1px solid rgba(255,255,255,.15);border-radius:8px;width:32px;height:28px;line-height:26px;font-size:18px;cursor:pointer;opacity:.35}.word-popover__btn:hover{opacity:.9}.word-popover__btn:hover{background:#ffffff29}.word-popover__btn:active{transform:translateY(1px)}.custom-captions-overlay-controlled{max-width:80vw;display:flex;flex-wrap:wrap;gap:6px;cursor:auto} ');
...
(function () {
  ...
  function useDraggable(ref) {
    ...
    const drag = reactExports.useRef({
      pointerId: -1,
      startX: 0,
      startY: 0,
      startLeft: 0,
    ...
    const clampIntoViewport = (el, left, top) => {
    ...
    const onPointerDown = (e) => {
      ...
      if (interactive || e.pointerType === "mouse" && e.button !== 0 || !ref.current || !pos)
        return;
      const el = ref.current;
      drag.current = {
    ...
    const style = pos ? { position: "fixed", left: pos.left, top: pos.top, visibility: "visible" } : { position: "fixed", visibility: "hidden" };
    return { pos, setPos, bind, style };
  }
  const CAPTION_SEGMENT = ".ytp-caption-segment";
  const CAPTIONS_TEXT = ".captions-text";
  const CAPTION_WINDOW_CONTAINER = ".ytp-caption-window-container";
  const VIDEO_PLAYER = "video.html5-video-player";
  const MOVIE_PLAYER = "#movie_player video";
  const VIDEO = "video";
  const getCurrentCaptionText = () => {
    const segs = Array.from(
      document.querySelectorAll(CAPTION_SEGMENT)
    );
    if (segs.length > 0)
      return segs.map((el) => el.textContent ?? "").join(" ").trim();
    const fallback = document.querySelector(CAPTIONS_TEXT);
    return (fallback?.textContent ?? "").trim();
  };
  const tokenize = (text) => text.match(/[\p{L}\p{N}’'\-]+/gu) ?? [];
  const CustomCaptionsOverlay = reactExports.forwardRef(({ overlayStyle, children, ...rest }, ref) => {
  ...
  const STORAGE_KEY$1 = "TWIN_SUBS_YT_WORDS_V1";
  function addWordToLocal(text, ru, en) {
    try {
      ...
      localStorage.setItem(STORAGE_KEY$1, JSON.stringify(arr));
      return true;
    } catch {
      return false;
    }
  ...
  function toCSV(words, opts = {}) {
    ...
    for (const w of words) {
      const term = w.text ?? "";
      const def = definition === "ru" ? w.ru ?? "" : [w.ru, w.en].filter(Boolean).join(" / ");
  ...
  function WordChip({
    ...
    const [addedTick, setAddedTick] = reactExports.useState(false);
    const [copiedTick, setCopiedTick] = reactExports.useState(false);
    const [deletedTick, setDeletedTick] = reactExports.useState(false);
    let openTimer;
    let closeTimer;
    ...
    const handleAdd = () => {
      ...
      if (ok) {
        setAddedTick(true);
        window.setTimeout(() => setAddedTick(false), 1e3);
      }
    };
    const handleCopyAll = async () => {
      ...
      setCopiedTick(true);
      window.setTimeout(() => setCopiedTick(false), 1e3);
    };
    const handleDeleteAll = async () => {
      clearStoredWords();
      setDeletedTick(true);
      window.setTimeout(() => setDeletedTick(false), 1e3);
    };
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "span",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "word-chip", tabIndex: 0, children: text }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "span",
            {
              ...
              role: "tooltip",
              "data-no-drag": true,
              children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "word-popover__rows", children: [
                actions && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "word-popover__actions", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "button",
                    {
                      type: "button",
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "button",
                    {
                      ...
                      children: deletedTick ? "✔️" : "🗑️"
                    }
                  )
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "word-popover__row", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "word-popover__label", children: "EN:" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "word-popover__value", children: loading ? "…" : en })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "word-popover__row", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "word-popover__label", children: "RU:" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "word-popover__value", children: loading ? "…" : ru })
                ] })
              ] })
            }
          )
  ...
  function ensureApiKey() {
    ...
    _GM_setValue(STORAGE_KEY, normalized);
    return normalized;
  }
  async function translateText(text, {
    ...
    const messages = [
      {
        role: "system",
        content: "You are a precise translation engine. Translate the text from one language to multiple target languages without adding any extra information or commentary. Keep it concise, natural, child-friendly."
      },
      {
        role: "user",
  ...
  function CustomCaptionsOverlayControlled({
    words,
    ...rest
  }) {
    const [openWord, setOpenWord] = reactExports.useState(null);
    const [trMap, setTrMap] = reactExports.useState({});
    const controllers = reactExports.useRef({});
    const ensureTranslation = async (w) => {
      const isExist = trMap[w]?.en || trMap[w]?.ru || trMap[w]?.loading;
      if (isExist) return;
      setTrMap((m) => ({
        ...m,
        [w]: { ...m[w] ?? {}, loading: true, error: void 0 }
      }));
    ...
    const handleOpenChange = (word, open) => {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "custom-captions-overlay-controlled", ...rest, children: [
      words.map((w, i) => {
      ...
      words.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(
        WordChip,
        {
          text: "🪄",
          actions: false,
  ...
  const getYoutubeVideo = () => {
    return document.querySelector(VIDEO_PLAYER) || document.querySelector(MOVIE_PLAYER) || document.querySelector(VIDEO);
  };
  const wasPlaying = (v) => !v.paused && !v.ended && v.currentTime > 0;
  function useHoverPauseVideo(getVideo = getYoutubeVideo) {
    const pausedByUsRef = reactExports.useRef(false);
  ...
  function App() {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsx(
      CustomCaptionsOverlay,
      {
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsx(CustomCaptionsOverlayControlled, { words })
      }
    );
  }
  ReactDOM.createRoot(
